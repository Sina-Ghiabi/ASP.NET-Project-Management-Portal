...
{
    [AbpMvcAuthorize]
    public class FinancialController : PMPortalControllerBase
    {
        ...
        //            catch (Exception ex)
        //            {
        //                //model.ErrorText = "Error while saving the file.";
           ...
           string paymentType, int? sortType, string fourthLevelCode)
        {
            ...
            var fileName = "PMCO-PaymentRequests-" + DateTime.Now.ToString("yyyy-MM-dd--hh-mm-ss") + ".xlsx";
            ...
            using (var package = new ExcelPackage(file))
            {
                ...
                ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("PMCO Payment Requests List - " + DateTime.Now.ToShortDateString());
                ...
                worksheet.Cells[1, 3].Value = "Project Code";
                worksheet.Cells[1, 4].Value = "Registration Date";
                worksheet.Cells[1, 5].Value = "Last Modified Date";
                worksheet.Cells[1, 6].Value = "Fourth Level Code	";
                worksheet.Cells[1, 7].Value = "Payment Request ID";
                worksheet.Cells[1, 8].Value = "Description";
                worksheet.Cells[1, 9].Value = "Classification";
                worksheet.Cells[1, 10].Value = "Pay To";
                worksheet.Cells[1, 11].Value = "Remaining Credit";
                worksheet.Cells[1, 12].Value = "Requested Amount";
                worksheet.Cells[1, 13].Value = "Allocated";
                worksheet.Cells[1, 14].Value = "Currency";
                worksheet.Cells[1, 15].Value = "Payment Method";
                worksheet.Cells[1, 16].Value = "Cheque Date	";
                worksheet.Cells[1, 17].Value = "Requester Notes";
                ...
                package.Workbook.Properties.Author = "Nexora.Co";
                package.Workbook.Properties.Company = "PMCO";
            ...
            int? sortType, int[] paymentTypeId)
        {
            ...
            var items = result.Item1.Select(i => new TransactionViewModel
            {
                ...
                //RefrenceDescription = i.PaymentRequest != null ? i.PaymentRequest.PaymentType.ToString() + "Payment Request " : i.ReceiveBill.ReceiveBillType.ToString() + "Receive Bill "
            }).ToList();
            ...
            int? sortType, string paymentTypeId)
        {
            ...
            var fileName = "PMCO-Transactions-" + DateTime.Now.ToString("yyyy-MM-dd--hh-mm-ss") + ".xlsx";
            var outputDir = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Uploads");
            ...
            {
                // add a new worksheet to the empty workbook
                ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("PMCO Transactions List - " + DateTime.Now.ToShortDateString());
                worksheet.HeaderFooter.FirstFooter.LeftAlignedText = string.Format("Generated: {0}", DateTime.Now.ToShortDateString());
                worksheet.Row(1).Height = 15;
                worksheet.View.RightToLeft = true;
                ...
                worksheet.Cells[1, 2].Value = "Project";
                worksheet.Cells[1, 3].Value = "Project Code";
                worksheet.Cells[1, 4].Value = "Request Date";
                worksheet.Cells[1, 5].Value = "Registration Date";
                worksheet.Cells[1, 6].Value = "Actions";
                worksheet.Cells[1, 7].Value = "Fourth Level Code	";
                worksheet.Cells[1, 8].Value = "Payment Request ID";
                worksheet.Cells[1, 9].Value = "Activity Title";
                worksheet.Cells[1, 10].Value = "Classification";
                worksheet.Cells[1, 11].Value = "Pay To";
                worksheet.Cells[1, 12].Value = "Remaining Credit";
                worksheet.Cells[1, 13].Value = "Requested Amount";
                worksheet.Cells[1, 14].Value = "Amount";
                worksheet.Cells[1, 15].Value = "Currency";
                worksheet.Cells[1, 16].Value = "Payment Method";
                worksheet.Cells[1, 17].Value = "Cheque Date";
                worksheet.Cells[1, 18].Value = "Requester Notes";
                worksheet.Cells[1, 19].Value = "Final Notes";
                worksheet.Cells[1, 20].Value = "Reference Document Number";
        ...
        public async Task<ActionResult> ReceiveBillsExcel(int? id, string projectId, string currencyId, string receiveBillType, string from, string to, int? sortType)
        {
            ...
            var fileName = "PMCO-ReceiveBills-" + DateTime.Now.ToString("yyyy-MM-dd--hh-mm-ss") + ".xlsx";
            var outputDir = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/Uploads");
            ...
            {
                // add a new worksheet to the empty workbook
                ExcelWorksheet worksheet = package.Workbook.Worksheets.Add("PMCO Receive Bills List - " + DateTime.Now.ToShortDateString());
                worksheet.HeaderFooter.FirstFooter.LeftAlignedText = string.Format("Generated: {0}", DateTime.Now.ToShortDateString());
                worksheet.Row(1).Height = 15;
                worksheet.View.RightToLeft = true;
                ...
                worksheet.Cells[1, 4].Value = "Allocated";
                worksheet.Cells[1, 5].Value = "Balance";
                worksheet.Cells[1, 6].Value = "Receive Method";
        ...
        public async Task<IActionResult> PaymentRequestChangeStatus(int paymentRequestId, int status)
        {
            try
            {
                ...
                if (status == 2)
                {
                    var urlWithAccessToken = _configuration["Notifications:SlackWebhookUrl"];
                    if (!string.IsNullOrEmpty(urlWithAccessToken))
                    {
                        var client = new SlackClient(urlWithAccessToken);
                        var message = String.Empty;
                        message = String.Format("New payment request submitted" + "\n" +
                                                   "Submitted by: " + _userManager.GetUserByIdAsync(paymentRequest.CreatorUserId.Value).Result.FullName + "\n" +
                                                   "Project: " + paymentRequest.Project.Title + "\n" +
                                                   "Requested Amount: " + paymentRequest.Amount + "\n" +
                                                   "Pay To: " + paymentRequest.PayTo + "\n" +
        ...
        public async Task<IActionResult> RemainCreditImportExcel(IFormFile ExcelFile)
        {
            ...
            if (ExcelFile != null)
            {
                ...
                else { return Json("The selected file is not an Excel file"); } // If File Wasn't Excel Format , Returns Error Alert
            ...
            return Json("Excel file imported successfully");
        }
        ...
        public async Task<ActionResult> RemindCreditExportExcel(string ProjectIDs , string PayTo , string FourthLevelCode , int SortType , int ShowAll) //ShowAll Is Flag That Represent That If User Wants To See The Values That Has 0 CreditAmount And DebtAmount
        {
            ...
            // Create the file using the FileInfo object
            var FileName = "Remaining Credit - " + new PersianDateTime(DateTime.Parse(DateTime.Now.ToString())).ToLongDateTimeInt() + ".xlsx";
            var FileNameForReturn = "Remaining Credit - " + new PersianDateTime(DateTime.Parse(DateTime.Now.ToString())).ToShortDateString() + ".xlsx"; //This Format Of File Is Used At The End For Returning The File
            ...
            using (var Package = new ExcelPackage(ExcelFile))
            {
                ...
                worksheet.Cells[1, 3].Value = "Detail";
                worksheet.Cells[1, 4].Value = "Person Name";
                worksheet.Cells[1, 5].Value = "Remaining Debit";
                worksheet.Cells[1, 6].Value = "Remaining Credit";
